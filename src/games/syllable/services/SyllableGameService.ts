...
import type { GameLevel } from '../data/SyllableGameData';
...
export class SyllableGameService {
  private apiService: APIDataService;
  private static readonly ACTIVITY_ID = 9;
  ...
  isCorrectAnswer(selectedValue: number | string, level: GameLevel): boolean {
    const correctOption = level.options.find(option => option.isCorrect);
    return correctOption ? correctOption.value === selectedValue : false;
  }
  ...
  async registerInteraction(
    ...
    try {
      ...
      const interactionData: InteractionData = {
        studentId: studentId,
        activityId: SyllableGameService.ACTIVITY_ID,
        questionId,
        answer,
        timeSpent,
  ...
  getStudentId(): number {
    ...
    return typeof userId === 'number' ? userId : parseInt(String(userId)) || SyllableGameService.DEFAULT_STUDENT_ID;
  }
