...
import { HistoryData } from "../logic/ComDatesHistoryData";

export class ComDatesHistoryScene extends Phaser.Scene {
  ...
  constructor() {
    super({ key: "ComDatesHistoryScene" });
  ...
  preload() {
    ...
    this.load.image("mascot", "/assets/comDatesGame/mascot.png");
    ...
    HistoryData.forEach(({ image }, index) => {
      ...
      this.load.audio(
        "infoAudio_" + index,
        "/assets/comDatesGame/history/lines/" + "line" + (index + 1) + ".m4a",
      );
    });
  }
  ...
  private handleNext(): void {
    if (this.registry.get("currentChat") < HistoryData.length - 1) {
      ...
      this.scene.start("ComDatesGameScene");
    }
  }
