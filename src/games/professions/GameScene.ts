...
import type {
  ProfessionDragLevel,
  ProfessionIntroLevel,
} from "./data/ProfessionsGameData";
...
export class GameScene extends Phaser.Scene {
    ...
    if (
      ...
      this.currentLevel =
        data.currentLevel !== undefined
          ? data.currentLevel
          : this.registry.get("professionsCurrentLevel") || 0;
      this.score =
        data.score !== undefined
          ? data.score
          : this.registry.get("professionsScore") || 0;
    }
  }
  ...
  preload() {
    ...
    this.load.audio(
      "professor-sound",
      "/assets/professions/sounds/professor.mp3",
    );
    this.load.audio(
      "bombeiro-sound",
      "/assets/professions/sounds/bombeiro.mp3",
    );
    this.load.audio(
      "cozinheira-sound",
      "/assets/professions/sounds/cozinheira.mp3",
    );
    this.load.audio(
      "policial-sound",
      "/assets/professions/sounds/policial.mp3",
    );
  }
  ...
  create() {
    ...
    this.professionsGameService = new ProfessionsGameService(this);
      ...
      (
        ...
        if (dropZone && dropZone.getData) {
          ...
          if (workplace && this.draggedProfession) {
            const dragIndex =
              this.currentLevel -
              ProfessionsGameData.introLevels.length -
              ProfessionsGameData.questionLevels.length;
  ...
  private setupUI() {
    ...
    this.professionNameText = this.add
  ...
  private startIntroLevel(levelIndex: number) {
    ...
    if (!this.dudaImage || !this.dudaImage.scene) {
      this.dudaImage = this.add
        .image(140, 300, "professionsDuda")
        .setScale(0.4);
    }
    ...
    if (
      introLevel.professionType !== "duda" &&
      introLevel.professionType !== "fim"
    ) {
      const professionLegend = this.add
        .text(380, 230, introLevel.professionName.toUpperCase(), {
          fontSize: "28px",
          color: "#FF6B35",
          fontFamily: "Arial Black",
          align: "center",
      ...
      const professionImage = this.add
        .image(380, 350, introLevel.professionType)
        .setScale(0.25);
      ...
      if (workplace) {
        ...
        const workplaceLegend = this.add
          .text(620, 230, workplaceNames[workplace], {
            fontSize: "28px",
            color: "#FF6B35",
            fontFamily: "Arial Black",
            align: "center",
        ...
        const workplaceImage = this.add
          .image(620, 350, workplace)
          .setScale(0.45);
    ...
    if (introLevel.soundFile) {
      ...
      if (
        introLevel.professionType === "duda" &&
        introLevel.professionName === "Introdução"
      ) {
        soundKey = "intro-sound";
      } else if (
        introLevel.professionType === "fim" &&
        introLevel.professionName === "Parabéns!"
      ) {
        soundKey = "fim-sound";
      } else {
  ...
  private startQuestionLevel() {
    ...
    const questionIndex =
      this.currentLevel - ProfessionsGameData.introLevels.length;
  ...
  private setupDragAndDrop(
    professionImage: Phaser.GameObjects.Image,
    _dragLevel: ProfessionDragLevel,
  ) {
  ...
  private createWorkplaceZones(dragLevel: ProfessionDragLevel) {
    const shuffledWorkplaces = [...dragLevel.workplaceOptions].sort(
      () => Math.random() - 0.5,
    );
    ...
    shuffledWorkplaces.forEach((workplace, index) => {
      ...
      const workplaceName =
        workplaceNames[workplace] || workplace.toUpperCase();
      const legendText = this.add
        .text(x, y - 100, workplaceName, {
          fontSize: "24px",
          color: "#FF6B35",
          fontFamily: "Arial Black",
          align: "center",
      ...
      dropZoneGraphics.strokeRect(
        x - (containerWidth + 20) / 2,
        y - (containerHeight + 20) / 2,
        containerWidth + 20,
        containerHeight + 20,
      );
  ...
  private async handleDrop(
    ...
    correctWorkplace: string,
    workplaceContainer?: Phaser.GameObjects.Container,
  ) {
    if (this.isTransitioning) return;
    ...
    try {
      ...
      if (isCorrect) {
        await this.professionsGameService.registerCorrectAnswer(
          studentId,
          questionId,
          selectedWorkplace,
        );
      } else {
        await this.professionsGameService.registerIncorrectAnswer(
          studentId,
          questionId,
          selectedWorkplace,
        );
  ...
  private async selectProfession(
  ...
  private goToNextLevel() {
    ...
    const isEndOfQuestionLevels =
      this.currentLevel + 1 === totalIntroLevels + totalQuestionLevels;
