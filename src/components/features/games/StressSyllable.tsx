...
const StressSyllable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (!gameRef.current.registry.has("syllableCurrentLevel")) {
        gameRef.current.registry.set("shouldRestartSyllable", true);
      }
    }
  }, [user]);
    ...
    return (
        <>
            ...
            <Header />
            <BackButton />
            <div id="game-container" className="relative" style={{ width: 800, height: 600 }} ></div>
            </div>
            <Footer />
        </>
